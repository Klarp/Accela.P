...
module.exports = {
	...
	async execute(message, args) {
		...
		if (!findUser && !args[0]) message.channel.send(`No link found: use ${prefix}link [osu user] to link your osu! account!`);

		try {
			const user = await osuApi.getUser({ m: 2, u: name });
			sendEmbedMessage(user, verified, message, 'ctb', name);
		} catch (e) {
