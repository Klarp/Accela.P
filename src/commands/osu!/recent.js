		...
		osuApi.getUserRecent({ u: search }).then(async r => {
			...
			curl.get(`https://osu.ppy.sh/osu/${recent.beatmapId}`, async function(err, response, body) {
				...
				if (recent.rank == 'F') {
					...
					const osuFailEmbed = new EmbedBuilder()
						.setAuthor({ name: recent.user.name || name, iconURL: `http://a.ppy.sh/${recent.user.id}`, url: `https://osu.ppy.sh/u/${recent.user.id}` })
						.setColor('#af152a')
...
${verified}`)
						...
						.setFooter({ text: `Completed ${rDate}` });
					message.channel.send({ embeds: [osuFailEmbed] });
				} else {
					const osuEmbed = new EmbedBuilder()
						.setAuthor({ name: recent.user.name || name, iconURL: `http://a.ppy.sh/${recent.user.id}` })
						.setColor('#af152a')
						.setTitle(`${recent.beatmap.artist} - ${recent.beatmap.title} [${recent.beatmap.version}]`)
						.setDescription(`${rank} | ${diffCalc} ${star[0]}★ | ${score} | {${hit300}/${hit100}/${hit50}/${hitmiss}}
